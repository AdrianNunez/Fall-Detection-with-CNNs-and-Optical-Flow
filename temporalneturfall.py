...
def saveFeatures(param, max_label, batch_size, phase, amount_of_data, parts, save_features, feature_extractor, classifier, features_file, labels_file, train_split, test_split):
        #data_folder = '/ssd_drive/data/'
        data_folder = '/home/anunez/project/UR_Fall_OF/'
        ...
        dataset_features_train = h5features.create_dataset('features', shape=(nb_total_stacks, num_features), dtype='float64')
        dataset_labels_train = h5labels.create_dataset('labels', shape=(nb_total_stacks, 1), dtype='float64')  
...
def main(learning_rate, batch_size, dropout, batch_norm, weight_0, weight_1, nb_neurons, exp, model_file, weights_file): 
   ...
   # w,b = classifier.get_layer('fc2').get_weights()
    ...
    e = EarlyStopping(monitor='loss', min_delta=0, patience=100, verbose=0, mode='auto')
    ...
    features_file = '/home/anunez/project/features/features_urfall_final3.h5'
    labels_file = '/home/anunez/project/labels/labels_urfall_final3.h5'
    features_file_test = '/home/anunez/project/features/features_fdd_final3.h5'
    labels_file_test = '/home/anunez/project/labels/labels_fdd_final3.h5'
    ...
    if do_training:
        ...
        for (train_index_falls, test_index_falls), (train_index_nofalls, test_index_nofalls) in zip(kf_falls.split(X_full[zeroes, ...]), kf_nofalls.split(X_full[ones, ...])):
            ...
            if cross_val:
                ...
                #X = np.asarray(h5features['features'])
                #_y = np.asarray(h5labels['labels'])
                stages = []
                for i in range(1,25):
            ...
            if batch_size == 0:
                history = classifier.fit(X,_y, validation_data=(X2,_y2), batch_size=X.shape[0], nb_epoch=6000, shuffle='batch', class_weight=class_weight)
            else:
                history = classifier.fit(X,_y, validation_data=(X2,_y2), batch_size=batch_size, nb_epoch=6000, shuffle='batch', class_weight=class_weight)
            #classifier.compile(optimizer=adam, loss='binary_crossentropy',  metrics=param['metrics'])
            
            #h5features_test = h5py.File(features_file_test, 'r')
            #h5labels_test = h5py.File(labels_file_test, 'r')
            #X_test = np.asarray(np.concatenate((h5features_test['train'],h5features_test['test']), axis=0))
            #_y_test = X = np.asarray(np.concatenate((h5labels_test['train'],h5labels_test['test']), axis=0))
...
if __name__ == '__main__':
    ...
    for learning_rate in [0.001, 0.01, 0.005, 0.0001]:
        for batch_size in [0, 128, 32, 16]: # en reserva 256,
